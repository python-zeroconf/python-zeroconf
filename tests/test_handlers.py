...
async def test_qu_response_only_sends_additionals_if_sends_answer():
    ...
    zc.cache.async_remove(a_record)
    a_record = info.dns_addresses()[0]
    ...
    zc.cache.async_remove(ptr_record)
    ...
    assert question.unicast is True
    query.add_question(question)
    zc.cache.async_add(info2.dns_pointer())  # Add 100% TTL for info2 to the cache
