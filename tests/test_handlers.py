...
async def test_response_aggregation_random_delay():
    ...
    now = current_time_millis()
    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (500, 600)):
    ...
    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (300, 400)):
    ...
    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (100, 200)):
    ...
    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (700, 800)):
...
async def test_future_answers_are_removed_on_send():
    ...
    now = current_time_millis()
    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (1, 1)):
        outgoing_queue.async_add(now, {info.dns_pointer(): set()})

    assert len(outgoing_queue.queue) == 1

    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (2, 2)):
        outgoing_queue.async_add(now, {info.dns_pointer(): set()})

    assert len(outgoing_queue.queue) == 2

    with unittest.mock.patch.object(_handlers, "MULTICAST_DELAY_RANDOM_INTERVAL", (1000, 1000)):
        outgoing_queue.async_add(now, {info2.dns_pointer(): set()})
        outgoing_queue.async_add(now, {info.dns_pointer(): set()})
